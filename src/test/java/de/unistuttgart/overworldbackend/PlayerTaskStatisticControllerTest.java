...
class PlayerTaskStatisticControllerTest {
    ...
    public void createBasicData() {
        ...
        initialMinigameTask = initialLecture.getWorlds().stream().findFirst().get().getMinigameTasks().stream().findFirst().get();
        ...
        fullURL = String.format("/lectures/%d/playerstatistics/"+initialPlayerstatistic.getUserId()+"/player-task-statistics", initialLecture.getId());
    ...
    void getTaskStatistics() throws Exception {
        PlayerTaskStatisticDTO statistic = playerTaskStatisticService.submitData(new PlayerTaskStatisticData(initialMinigameTask.getGame(),initialMinigameTask.getConfigurationId(),80,initialPlayerstatisticDTO.getUserId()));
        ...
        final List<PlayerTaskStatisticDTO> playerTaskStatisticDTOs = Arrays.asList(objectMapper.readValue(
